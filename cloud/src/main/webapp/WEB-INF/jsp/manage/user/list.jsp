					...
					<form class="layui-form layui-form-pane" action="" lay-filter="seachForm" id="seachForm">
						<div class="layui-form-item">
							...
							<div class="layui-inline">
								...
								<div class="layui-input-inline">
									 <select id="role" name="role"  lay-search></select>
								</div>
							</div>
							<div class="layui-inline">
        ...
        layui.use(['form', 'table'], function () {
            ...
            table.render({
                ...
                cols: [[
                    ...
                    { field: 'company', title: '企业', align: "center" },
            ...
            $.post("../role/roleList.do", { "mathRandom": Math.random() }, function (result) {
                if (result.status) {
                    $('#role').append(new Option('', ''));
                    for (var i = 0; i < result.data.length; i++) {
                        $('#role').append(new Option(result.data[i].name, result.data[i].id));
                    }
                    layui.form.render();
                }
            });
            ...
            table.on('tool(currentTableFilter)', function (obj) {
                ...
                if (obj.event === 'edit') {
                	...
                	layer.confirm('确定要重置吗？', function (index) {
                        $.post("./resetPwd.do", { 'id': data.id,mathRandom: Math.random()  }, function (res) {
                            layer.msg(res.msg);
                            reload();
                        })
                        layer.close(index);
