                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data == null) {
                            ...
                            for (var key in data.menuInfo) {
                                layuimini.handUrl(data.menuInfo[key]);
                            }
...
/**
         * 递归处理链接路径
         */
        this.handUrl = function(obj) {
            var that = this;
            obj.child = $.map(obj.child,function(item, index) {
                item.icon = item.icon ? item.icon : 'fa fa-list-alt';
                if (item.href) {
        ...
        this.initBgColor = function() {
            ...
            x.innerHTML = 'x<style>' + styleHtml + '</style>';
