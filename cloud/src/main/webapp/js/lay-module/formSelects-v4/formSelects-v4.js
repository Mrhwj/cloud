	...
	Common.prototype.one = function(target){//一次性事件绑定
		...
		$(target ? target : document).off('click', `dl.${DL}`).on('click', `dl.${DL}`, function(e) {
			...
			if(treeId){
				...
				if(childrens && childrens.length){
					...
					len ? (
						...
						dd.find('>i').remove(),
						(childrens = childrens.filter(function(index, item){ $(item).attr('tree-id').split('-').length - 1 == treeId.split('-').length}))
					);
					childrens.animate({
						height: len
					}, 150)
					...
					if(treeConfig.nextClick && treeConfig.nextClick instanceof Function){
						treeConfig.nextClick(id, this.getItem(id, dd), function(res)  {
							if(!res || !res.length){
								this.handlerLabel(id, dd, !dd.hasClass(THIS));
							}else{
			...
			if(dd.hasClass(FORM_SELECT_TIPS)){//tips的处理
				...
				var obj = data[id].config.btns.filter(function(bean){ bean.name == method})[0];
