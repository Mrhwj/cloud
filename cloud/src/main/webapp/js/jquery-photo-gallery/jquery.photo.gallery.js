	...
	$.fn.extend({
		...
		photoGallery: function (options) {
			...
			function init() {
				...
				var realSrc = o.imgs[o.activeIndex].url.replace(/_small/, '')
				$image = $(".image[index='" + o.activeIndex + "']", $gallery).addClass("active").attr('src', realSrc);
			...
			function toggleImage() {
				...
				$image = $(".image[index='" + o.activeIndex + "']", $gallery).addClass("active").css({
					width: imageWidth,
					height: imageHeight
				}).removeClass("rotate0 rotate90 rotate180 rotate270").attr('src', realSrc);
				$thumbImg = $thumbnails.find("img").attr("src", o.imgs[o.activeIndex].url);
				$thumbnails.find("img").removeAttr("class").removeAttr("style");
				$gallery.find('.pageIndex').html((Number(o.activeIndex) + 1) + '/' + o.imgs.length)
				isVertical = false;
				$thumbnails.hide();
	...
	$.extend({
		...
		openPhotoGallery: function (obj) {
			...
			$("<iframe></iframe").appendTo("body")
				...
				.css({
					position: "fixed",
					left: "5%", //($(window).width() - winWidth) / 2,
					top: '5%', //($(window).height() - winHeight) / 2,
					width: '90%',
					height: '90%',
